...
import { loadLiquidBalance, loadNetWorth, loadMonthlyIncome, loadMonthlySurplus } from './signal-loaders';
...
describe('signal loaders', () => {
  it('liquidBalance sums only checking + savings balances', async () => {
    ...
    const v = await loadLiquidBalance(2);
    expect(v).toBe(4200);
  ...
  it('netWorth sums all non-deleted balances', async () => {
    ...
    expect(await loadNetWorth(2)).toBe(90000);
  });
  ...
  it('monthlyIncome averages income over the trailing window', async () => {
    ...
    expect(await loadMonthlyIncome(2)).toBe(2000);
  });
  ...
  it('monthlySurplus averages net (income - expense) over the window', async () => {
    ...
    // (12000 - 6000) / 6
    expect(await loadMonthlySurplus(2)).toBe(1000);
  });
});
