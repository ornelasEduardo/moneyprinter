...
export default function IncomeBudgetPage() {
  ...
  useEffect(() => {
    function handleClickOutside(event: MouseEvent) {
      if (
        paycheckRef.current &&
        !paycheckRef.current.contains(event.target as Node)
      ) {
    ...
    async function loadSources() {
      try {
        ...
        if (sources.length > 0) {
          ...
          const defaultSource =
            sources.find((s) => s.name === "Primary Paycheck") || sources[0];
  ...
  useEffect(() => {
    ...
    async function loadData() {
      setLoading(true);
      try {
        const data = await getBudgetsForIncomeSource(
  ...
  useEffect(() => {
    if (viewMode === "csv") {
      const csv = budgets
        .map((a) => {
          const valStr =
            a.unit === "percentage" ? `${a.value}%` : `$${a.value}`;
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      toastError("Failed to save configuration. Please try again.");
    } finally {
      setSaving(false);
    }
  };
    ...
    () => [
      {
        ...
        cell: (info) => (
          <Input
            ...
            onChange={(e) =>
              updateBudget(info.row.original.id, "name", e.target.value)
            }
            placeholder="Name"
          />
        ),
      ...
      },
      {
        ...
        cell: (info) => (
          <Select
            value={info.getValue() as string}
            onChange={(e) =>
              updateBudget(info.row.original.id, "unit", e.target.value)
            }
            options={[
      ...
      },
      {
        ...
        cell: (info) => (
          <Input
            ...
            placeholder="0.00"
            startAdornment={
              info.row.original.unit === "fixed" ? "$" : undefined
            }
            endAdornment={
              info.row.original.unit === "percentage" ? "%" : undefined
            }
          />
        ),
        size: 150,
      },
      {
        ...
        cell: (info) => (
          <Select
            value={info.getValue() as string}
            onChange={(e) =>
              updateBudget(info.row.original.id, "type", e.target.value)
            }
            options={[
  ...
  return (
    <Page>
      <header style={{ marginBottom: "2rem", position: "relative" }}>
        ...
        <div style={{ textAlign: "center" }}>
          <Text variant="h1" weight="bold" align="center" className="mb-2">
            Income Budget
          </Text>
          <Text color="muted" align="center">
            Define how your paycheck is distributed
          </Text>
        </div>
      </header>
      ...
      <Grid columns="1fr 350px" gap={8}>
        ...
        <Flex direction="column" gap={8}>
            ...
            <div ref={paycheckRef}>
              {isEditingPaycheck ? (
                ...
                >
                  $
                  {paycheckAmount
                    ? parseFloat(paycheckAmount).toFixed(2)
                    : "0.00"}
                  <Pencil
                    size={14}
                    strokeWidth={2.5}
          ...
          <Tabs
            value={viewMode}
            onValueChange={(val) => setViewMode(val as "table" | "csv")}
            className="w-full"
          >
            <Flex
                ...
                <Card style={{ borderTopLeftRadius: 0 }}>
                  <div style={{ padding: "1rem" }}>
                    <Text color="muted" variant="small" className="mb-2 block">
                      Edit your budgets as CSV. Format:{" "}
                      <code>Name, Amount, Type</code>
                    </Text>
                    <Text color="muted" variant="small" className="mb-4 block">
                      Example: <code>401k, 20%, savings</code> or{" "}
                      <code>Netflix, $15, expense</code>
                    </Text>
                    <Textarea
                      value={csvContent}
            ...
            <Flex direction="column" gap={6} style={{ marginBottom: "2rem" }}>
              ...
              <Flex justify="space-between" align="center">
                ...
                <Text
                  ...
                  color={remaining < 0 ? "error" : undefined}
                >
                  ${remaining.toFixed(2)}
                </Text>
              </Flex>
