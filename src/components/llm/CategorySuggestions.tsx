...
import { Badge, Button, Card, Flex, Input, Spinner, Stack, Text, useToast } from 'doom-design-system';
...
export default function CategorySuggestions() {
  ...
  const run = async () => {
    ...
    try {
      ...
      if (r.length === 0) toastSuccess('No untagged transactions to categorize.');
    } catch (e) {
    ...
    <Card className={styles.card}>
      <Flex align="center" justify="space-between" wrap gap={3}>
        ...
        <Button variant="primary" onClick={run} disabled={loading} data-testid="llm-suggest">
          {loading ? <><Spinner size="sm" /> Categorizing…</> : <><Sparkles size={16} strokeWidth={2.5} /> Suggest categories</>}
        </Button>
      </Flex>
      ...
      {rows && rows.length === 0 && !loading && (
        <Text color="muted">Nothing to categorize right now — every recent transaction already has a tag.</Text>
      )}
    </Card>
  );
}
